...
def process_organization(org_name):
    ...
    def write_to_csv(workspaces_data, total_resources, total_applies_per_month):
        with open(f'{org_name}_terraform_metrics.csv', mode='w', newline='') as file:
            writer = csv.writer(file)
            header = ['Workspace Name', 'Resource Count'] + [f'{month} Applies' for month in past_13_months] + ['Organization Name']
            writer.writerow(header)
            for workspace in workspaces_data:
                row = [workspace['name'], workspace['resource_count']] + [workspace['applies'][month] for month in past_13_months] + [org_name]
                writer.writerow(row)
            total_row = ['Total', total_resources] + [total_applies_per_month[month] for month in past_13_months] + [org_name]
